  ...
  return (
    <div className="rounded-2xl border border-border/30 bg-[hsl(var(--surface-1))]/70 p-5 shadow-soft backdrop-blur">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Mini serverlar</p>
          <p className="text-sm text-muted-foreground">Shaxsiy instansiyalar holatini kuzating.</p>
        </div>
      ...
      {!items.length ? (
        <div className="rounded-xl border border-dashed border-border/40 bg-[hsl(var(--surface-2))]/60 p-4 text-sm text-muted-foreground">
          Hozircha faol mini serverlar yoʻq. Mini Services sahifasidan yangi instansiyani ishga tushiring.
        </div>
      ) : (
              ...
              >
                ...
                <div className="relative flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 text-xs font-semibold">
                    ...
                    >
                      {isOnline ? 'Onlayn' : isMaintenance ? 'Texnik xizmat' : 'Oflayn'}
                    </span>
                ...
                <div className="relative mt-3 grid grid-cols-3 gap-2 text-[11px] text-muted-foreground">
                  ...
                  <span>Kechikish {server.latency_ms ?? '—'}ms</span>
                  <span>Zona {server.region ?? 'edge'}</span>
                </div>
              </div>
            );
          })}
