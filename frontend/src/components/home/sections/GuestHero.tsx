        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr,0.9fr]">
          <div className="space-y-7 rounded-[var(--radius-md)] border border-white/5 bg-[hsl(var(--card))]/75 p-8 shadow-[0_25px_90px_hsla(var(--primary)/0.18)] backdrop-blur-xl dark:border-border/60">
            ...
            <div className="rounded-[var(--radius-md)] border border-border/60 bg-[hsl(var(--surface))]/80 p-4 shadow-sm">
              ...
              <p className="mt-2 text-xs text-muted-foreground">Faolligingiz oshgani sari badge va ustozlik imkoniyatlari ochiladi.</p>
            </div>
          </div>
          <div className="space-y-5">
            <div className="rounded-[var(--radius-md)] border border-white/10 bg-[hsl(var(--card))]/70 p-6 shadow-[0_18px_45px_hsla(var(--primary)/0.14)] backdrop-blur-xl dark:border-border/70">
              ...
              <div className="mt-5 flex flex-wrap gap-2 text-xs">
                {heroTags.length ? (
                  heroTags.map((tag) => (
                    <span key={tag.slug ?? tag.name} className="rounded-full border border-border/60 bg-[hsl(var(--surface))] px-3 py-1 text-[hsl(var(--foreground))]">
                      #{tag.name}
                    </span>
                  ))
                ) : (
                  <span className="rounded-full border border-dashed border-border/60 px-3 py-1 text-muted-foreground">Yangi teglar kutilmoqda</span>
                )}
              </div>
            </div>
            <div className="mt-4">
